...
def stub_download(monkeypatch):
    async def fake_download(client, url, target_size, output_name):
        ...
        shade = (abs(hash(url)) % 200, 80, 120)
        return _jpg(colour=shade)
...
def test_the_beats_own_query_ladder_is_reused(
    tmp_path, stub_search, stub_download, stub_selection, monkeypatch
):
    """Not a new phrasing scheme -- the same rungs the Pexels path climbs."""
    calls = stub_search([])
    ...
    assert [call["query"] for call in calls] == ladder
